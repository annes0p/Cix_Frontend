...
function EstadoBadge({ stockActual, stockMinimo }) {
    if (stockActual === 0) {
        return <span className="px-2 py-1 rounded-md text-xs font-semibold bg-red-100 text-red-700">Sin stock</span>;
    }
    if (stockActual < stockMinimo) {
        return <span className="px-2 py-1 rounded-md text-xs font-semibold bg-orange-100 text-orange-700">Stock bajo</span>;
    }
    return <span className="px-2 py-1 rounded-md text-xs font-semibold bg-green-100 text-green-700">Óptimo</span>;
}

function StockValor({ stockActual, stockMinimo }) {
    if (stockActual === 0) return <span className="font-bold text-red-600">{stockActual}</span>;
    if (stockActual < stockMinimo) return <span className="font-bold text-orange-500">{stockActual}</span>;
    return <span className="font-bold text-green-600">{stockActual}</span>;
}

export default function InventarioTabla({ productos, pagina, totalPaginas, onPaginaChange }) {
    if (productos.length === 0) {
        return (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                <p className="text-gray-400 text-sm">No se encontraron productos con los filtros aplicados.</p>
            </div>
        );
    }
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                        ...
                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Almacén</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock actual</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock mínimo</th>
                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Estado</th>
                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Última actualización</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    {productos.map((producto, i) => (
                        <tr key={producto.id || i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3 text-center">
                                <StockValor stockActual={producto.stockActual} stockMinimo={producto.stockMinimo} />
                            </td>
                            <td className="px-4 py-3 text-center text-gray-600">{producto.stockMinimo}</td>
                            <td className="px-4 py-3">
                                <EstadoBadge stockActual={producto.stockActual} stockMinimo={producto.stockMinimo} />
                            </td>
                            <td className="px-4 py-3">
