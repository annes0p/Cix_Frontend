...
export default function UltimasVentas({ salesData }) {
    ...
    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between mb-4 border-b border-gray-50 pb-2">
          <h3 className="text-sm font-bold text-cixoil-red flex items-center gap-2"><span>🛒</span> Últimas ventas</h3>
          <button className="text-xs font-bold text-cixoil-red hover:underline flex items-center gap-0.5 group">Ver todas <span className="group-hover:translate-x-0.5 transition-transform">→</span></button>
        </div>
            ...
            <tbody className="divide-y divide-gray-50 text-xs font-semibold text-gray-700">
              {sales.map((sale, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-3 font-bold text-gray-900 truncate max-w-[130px]">{sale.client}</td>
                  <td className="py-3 text-gray-400 font-medium leading-tight text-[11px]">
                    <div>{sale.date}</div>
                    <div className="text-[10px] text-gray-400/70">{sale.time}</div>
                  </td>
                  ...
                  <td className="py-3 text-center">
                    <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-md border ${getStatusClass(sale.status)}`}>{sale.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
