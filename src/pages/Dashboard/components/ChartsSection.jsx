...
export default function ChartsSection({ chartData }) {
  ...
  const pieData = chartData?.pieData || [
    ...
    { name: 'Químicos', value: 10, color: '#D1A319' },
    { name: 'Otros', value: 5, color: '#4D4D4D' },
  ];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col">
        ...
        {/* Contenedor padre calibrado en ancho y alto */}
        <div className="w-full h-64 text-[10px] font-bold">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={lineData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
              ...
              <YAxis stroke="#9CA3AF" tickLine={false} axisLine={false} tickFormatter={(v) => `$ ${(v / 1000000).toFixed(1)}M`} />
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between">
        ...
        <div className="relative w-full h-40 flex items-center justify-center">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" innerRadius={50} outerRadius={68} paddingAngle={3} dataKey="value">
          ...
          <div className="absolute text-center flex flex-col justify-center">
            ...
            <span className="text-sm font-black text-gray-900 tracking-tight">$ 18.456.700</span>
          </div>
        </div>
