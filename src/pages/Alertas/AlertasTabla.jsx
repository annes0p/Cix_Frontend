function EstadoBadge({ stock, minStock }) {
    if (stock === 0) {
        return (
        ...
        return (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                <p className="text-gray-400 text-sm">
                    No existen alertas activas. ¡Todo el stock está en orden!
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
                ...
                <tbody>
                    {alertas.map((alerta) => {
                        const stock = alerta.stock ?? alerta.quantity ?? 0;
                        const minStock =
                            alerta.minStock ?? alerta.minimumStock ?? 0;
                        return (
                            <tr
                                key={alerta.id}
                            ...
                            >
                                <td className="px-4 py-3 font-semibold text-gray-900">
                                    {alerta.name || alerta.nombre}
                                </td>
                                <td className="px-4 py-3 text-gray-600">
                                    {alerta.categoryName || alerta.categoria}
                                </td>
