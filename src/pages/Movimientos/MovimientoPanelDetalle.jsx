import { X, Printer, Eye, Phone, User, Calendar, CreditCard, MapPin } from "lucide-react";
...
function EstadoBadge({ estado }) {
    ...
    return (
        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${estilos[estado] ?? "bg-gray-100 text-gray-600"}`}>
            {estado}
        </span>
    );
}
        ...
        <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-2">
                <div>
                    <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-gray-900 text-sm">{mov.id}</span>
                        <EstadoBadge estado={mov.estado} />
                    </div>
                    {mov.factura && (
                        <p className="text-xs text-gray-400 mt-0.5">Factura: {mov.factura}</p>
                    )}
                    <span className={`inline-block mt-1.5 px-2.5 py-0.5 rounded text-xs font-medium ${mov.tipo === "Venta" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                        {mov.tipo}
                    </span>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 text-sm">
                ...
                <section>
                    ...
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            ...
                            <div>
                                <p className="font-medium text-gray-900 leading-tight">{mov.cliente}</p>
                ...
                <section>
                    ...
                    <div className="space-y-2">
                        <InfoRow icon={Calendar} label="Fecha" value={formatFecha(mov.fecha)} />
                        <InfoRow icon={User} label="Vendedor" value={mov.vendedor} />
                        <InfoRow icon={CreditCard} label="Condición de pago" value={mov.condicionPago} />
                        <InfoRow icon={MapPin} label="Dirección de entrega" value={mov.direccionEntrega} />
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="border border-gray-100 rounded-lg overflow-hidden">
                        ...
                        {mov.productos.map((p, i) => (
                            <div key={i} className="grid grid-cols-4 gap-1 px-3 py-2 border-t border-gray-100 text-xs">
                                <div className="col-span-2">
                                    <p className="font-medium text-gray-800 leading-tight">{p.nombre}</p>
                                    <p className="text-gray-400">{p.descripcion}</p>
                                </div>
                                <span className="text-right text-gray-600 self-center">{p.cantidad}</span>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex gap-2">
                ...
                <button className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-cixoil-red text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
                    <Eye size={15} />
                    Ver factura
                </button>
            </div>
