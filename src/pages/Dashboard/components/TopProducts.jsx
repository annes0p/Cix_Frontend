  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col h-full justify-between">
      ...
      <div className="space-y-3.5 flex-1 flex flex-col justify-center">
        {products.map((p) => (
          <div key={p.id} className="flex items-center justify-between text-xs group cursor-pointer hover:bg-slate-50/80 p-1.5 rounded-lg transition-colors">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-gray-100 rounded-md border border-gray-100 flex items-center justify-center p-1 shrink-0">
                <img src={PremiumIcon} alt={p.name} className="w-full h-full object-contain" />
              </div>
            ...
            <span className="font-black text-sm text-cixoil-green bg-green-50/80 px-2.5 py-1 rounded-md min-w-[32px] text-center">
              {p.qty}
            </span>
          </div>
        ))}
