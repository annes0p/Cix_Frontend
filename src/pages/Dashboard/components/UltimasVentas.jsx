export default function UltimasVentas({ ventas }) {
    ...
    const getStatusClass = (status) => {
        switch (status) {
            case 'COMPLETED': return 'bg-green-50 text-cixoil-green border-green-100';
            case 'CANCELLED': return 'bg-red-50 text-cixoil-red border-red-100';
    ...
    if (!ultimas.length) {
        return (
            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm h-full flex flex-col justify-center items-center gap-2">
                <p className="text-sm font-bold text-gray-400">No hay ventas registradas</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col h-full">
            <div className="flex items-center justify-between mb-4 border-b border-gray-50 pb-2">
                <h3 className="text-sm font-bold text-cixoil-red">Ultimas ventas</h3>
            </div>
                        ...
                        {ultimas.map((venta) => (
                            <tr key={venta.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="py-3 font-bold text-gray-900 truncate max-w-[130px]">
                                    {venta.client?.name || venta.clientName || '-'}
                                </td>
                                <td className="py-3 text-gray-400 font-medium text-[11px]">
                                    {venta.saleDate || venta.date || '-'}
                                </td>
                                <td className="py-3 font-black text-gray-900 tracking-tight">
                                    S/. {venta.total || '-'}
                                </td>
                                <td className="py-3 text-center">
                                    <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-md border ${getStatusClass(venta.status)}`}>
